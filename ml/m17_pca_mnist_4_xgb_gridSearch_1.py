# m13으로 만든 0.95 이상의 n_component를 사용하여 xgb 모델을 만들 것 (디폴트)
# mnist DNN보다 성능이 좋도록
# RandomSearch로도 해볼 것
...
#2 Modeling    
model = RandomizedSearchCV(XGBRegressor(), PARAMETERS, cv=kfold, verbose=1)   # cv가 5회(N_SPLITS) # cross_validation?
# model = SVC()
...
print("accuracy_score : ", accuracy_score(y_test, np.argmax(y_pred)))
