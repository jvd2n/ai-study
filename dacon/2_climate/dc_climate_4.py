...
def data_preprocessing(data, target_columns):
    ...
    data['요약문_연구목표'] = data.apply(
        lambda x: x['과제명'] if x['요약문_연구목표'] == 'NONE' else x['요약문_연구목표'], axis=1)

    okt = Okt()
    data['요약문_한글키워드'] = data.apply(lambda x: ','.join(
        okt.nouns(x['과제명'])) if x['요약문_한글키워드'] == 'NONE' else x['요약문_한글키워드'], axis=1)

    data.loc[:, target_columns] = data[target_columns].applymap(
...
def oversampling_minor_classes(train, target_columns):
    temp = train.copy()
    temp.loc[:, target_columns] = temp.loc[:,
                                           target_columns].applymap(lambda x: len(x.split()))
    pj_range = (6, 11)
    summ_goal_range = (30, 89)
    summ_key_range = (5, 9)
    temp = temp.query('label != 0')
    temp = pd.DataFrame(list(train.loc[temp.query(
        '과제명 in @pj_range or 요약문_연구목표 in @summ_goal_range or 요약문_한글키워드 in @summ_key_range').index].values)*1, columns=temp.columns)
...
def vectorize_data(train, test, top_ks, target_columns):
    ...
    for top_k, column in zip(top_ks, target_columns):
        train_input, test_input = ngram_vectorize(
            train[column], train['label'], test[column], min(80000, top_k))
...
for train_idx, valid_idx in skf.split(train_inputs[0], labels):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            ckpt_path, monitor='val_acc', save_best_only=True, save_weights_only=True),
        tf.keras.callbacks.ReduceLROnPlateau(
            monitor='val_loss', factor=0.9, patience=2,),
    ]
    model = create_model(input_shape0, input_shape1,
                         input_shape2, num_labels, learning_rate)
    model.fit(
...
# # tf.keras.utils.plot_model(model, dpi = 500)
