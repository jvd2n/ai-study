...
PARAMETERS = [
    {'C': [1, 10, 100, 1000], 'kernel': ['linear'], 'degree': [3, 4, 5]},                            # 4 x 5(cv) = 20
    {'C': [1, 10, 100], 'kernel': ['rbf'], 'gamma': [0.001, 0.0001]},           # 3 x 1 x 2 x 5(cv) = 30
    {'C': [1, 10, 100, 1000], 'kernel': ['sigmoid'], 'gamma': [0.01, 0.001, 0.0001], 'degree': [3, 4]}, # 4 x 1 x 2 x 5(cv) = 40
]                                                                               # 90회
#2Modeling        
# model = GridSearchCV(SVC(), PARAMETERS, cv=kfold, verbose=1)   # cv가 5회(N_SPLITS) # cross_validation?
