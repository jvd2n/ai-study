...
data = pd.read_csv('./dacon/_data/4_agriculture/public_data/train.csv')
...
save_path = f'./dacon/4_agriculture/best_model.pt' # 모델 저장 경로
...
print(sample_batch['encoder_input'].shape, sample_batch['decoder_input'].shape)
...
submission = pd.read_csv('./dacon/_data/4_agriculture/sample_submission.csv')
...
for date in public_date_list:
    test_df = pd.read_csv(f'./dacon/_data/4_agriculture/public_data/test_files/test_{date}.csv')
    data = pd.read_csv('./dacon/_data/4_agriculture/public_data/train.csv')
    ...
    submission.loc[idx, '배추_가격(원/kg)':] = output[0,[6,13,20]].numpy()
    
submission.to_csv('./dacon/_output/4_agri/agri_dacon_baseline.csv', index=False)
