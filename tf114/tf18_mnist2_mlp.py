...
b1 = tf.Variable(tf.random.normal([1, 128], stddev=0.1, name='bias1'))
# hyppthesis = x * w + b
layer1 = tf.nn.relu(tf.matmul(x, w1) + b1)

# Hidden Layer
w2 = tf.Variable(tf.random.normal([128, 96], stddev=0.1, name='weight2'))
b2 = tf.Variable(tf.random.normal([1, 96], stddev=0.1, name='bias2'))
# hyppthesis = x * w + b
layer2 = tf.nn.relu(tf.matmul(layer1, w2) + b2)

# Hidden Layer
w3 = tf.Variable(tf.random.normal([96, 48], stddev=0.1, name='weight3'))
b3 = tf.Variable(tf.random.normal([1, 48], stddev=0.1, name='bias3'))
# hyppthesis = x * w + b
layer3 = tf.nn.relu(tf.matmul(layer2, w3) + b3)

# Output Layer
w4 = tf.Variable(tf.random.normal([48, 10], stddev=0.1, name='weight4'))
