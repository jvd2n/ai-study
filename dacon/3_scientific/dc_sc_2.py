...
train_df = pd.read_csv(os.path.join(
    '.', 'dacon', '_data', '3_scientific', 'train.csv'))
...
mols_df = pd.DataFrame().from_dict(
    {n: v.calcdesc()for n, v in mols.items()}).transpose()
...
print(
    f'MSE : {mean_squared_error(y_test,y_pred):.6} / R2 : {r2_score(y_test,y_pred):.6}')
